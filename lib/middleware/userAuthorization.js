...
module.exports = function userAuthorization(server, options, parse, validate) {
  ...
  return function userAuthorization(req, res, next) {
    ...
    function validated(err, consumer, callbackURL, areq) {
      ...
      req.session.oauth.client =
      req.session.oauth.consumer = consumer;
      req.session.oauth.callbackURL = callbackURL;
      ...
      server.serializeClient(consumer, function(err, obj) {
        ...
        txn.req = areq;
        txn.authz = req.session.oauth.authz;
    ...
    parse(req, function(err, params) {
      ...
      req.session.oauth.authz = params || {}
      req.session.oauth.authz.token = token;
      ...
      req.session.oauth.authz.callbackURL = callback || req.session.callback;
      console.log(req.session.oauth.authz);

      var arity = validate.length;
      if (arity == 3) {
        validate(token, req.session.oauth.authz, validated);
      } else { // arity == 2
