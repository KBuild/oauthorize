 ...
 *
 *     req.session.oauth.transactionID  an ID assigned to this transaction
 *     req.session.oauth.consumer       the consumer instance supplied by the application to the `validate` callback
 *     req.session.oauth.client         alias for `req.session.oauth.consumer`
 *     req.session.oauth.callbackURL    URL to redirect the user to after authorization
 *     req.session.oauth.req            parameters from the initial request token request made by the client
 *     req.session.oauth.authz          parameters parsed from this user authorization request
 *     req.session.oauth.user           the authenticated user who allowed or denied the authorization request
 *     req.session.oauth.res            the users decision, including `allow` flag and other parameters parsed by the implementation
 *     req.session.oauth.verifier       the verifier required to obtain an access token
 *
...
module.exports = function userDescision(server, options, parse, issue) {
  ...
  return function userDescision(req, res, next) {
    ...
    if (!req.session.oauth) { return next(new Error('OAuth transaction not found.')); }
    
    parse(req, function(err, ares) {
      if (err) { return next(err); }
      
      var tid = req.session.oauth.transactionID;
      req.session.oauth.user = req.session.uid;//load user id from session
      req.session.oauth.res = ares || {};
      
      if (req.session.oauth.res.allow === undefined) {
        if (!req.body[cancelField]) { req.session.oauth.res.allow = true; }
        else { req.session.oauth.res.allow = false; }
      }
      
      var allow = req.session.oauth.res.allow;
      ...
      if (allow) {
        function issued(err, verifier) {
          ...
          if (!req.session.oauth.callbackURL || req.session.oauth.callbackURL == 'oob') {
            req.session.oauth.verifier = verifier;
            removeTxnLater();
            return next();
          }
        
          var parsed = url.parse(req.session.oauth.callbackURL, true);
          delete parsed.search;
          parsed.query['oauth_token'] = req.session.oauth.authz.token;
        ...
        if (arity == 4) {
          issue(req.session.oauth.authz.token, req.session.oauth.user, req.session.oauth.res, issued);
        } else { // arity == 3
          issue(req.session.oauth.authz.token, req.session.oauth.user, issued);
        }
      } else {
        ...
        // checking if `req.session.oauth.res.allow` is `false` and rendering a message
