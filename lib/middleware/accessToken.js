 ...
 * Assumptions:
 *
 * `req.session` must be set on the request and contain `oauth.token` and
...
module.exports = function accessToken(options, verify, issue) {
  ...
  return function accessToken(req, res, next) {
    if (!req.session) { return next(new Error('authentication info not available')); }
    
    var consumer = req[userProp]
      , requestToken = req.session.oauth.token
    ...
    function verified(err, ok) {
      ...
      if (arity == 3) {
        ...
        issue(consumer, requestToken, req.session, issued);
      }
    }
    ...
    if (arity == 3) {
      ...
      verify(requestToken, verifier, req.session, verified);
    }
  }
}
