...
module.exports = function accessToken(options, verify, issue) {
  ...
  return function accessToken(req, res, next) {
    ...
    var headers = utils.parseHeader(req.headers);
    
    var consumer = req[userProp];
    var verifier = req.body.oauth_verifier || headers['oauth_verifier'];
    var requestToken = headers['oauth_token'];//can be vulnerable
    ...
    function verified(err, ok) {
      ...
      if (arity == 3) {
        ...
        issue(consumer, requestToken, authInfo, issued);
      }
    }
    ...
    if (arity == 3) {
      ...
      verify(requestToken, verifier, authInfo, verified);
    }
  }
}
